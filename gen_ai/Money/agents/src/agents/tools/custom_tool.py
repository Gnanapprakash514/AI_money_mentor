...
def _safe_eval(v: Any) -> float:
    """Coerce a value to float, safely evaluating simple arithmetic strings."""
    if isinstance(v, (int, float)):
        return float(v)
    if isinstance(v, str):
        try:
            node = ast.parse(v.strip(), mode="eval")
...
class SIPCalculatorInput(BaseModel):
    ...
    monthly_sip: float = Field(..., description="Monthly SIP amount in INR. Must be a plain number.")
    ...
    step_up_percent: float = Field(0.0, description="Annual SIP step-up percentage. Default is 0.")
    existing_corpus: float = Field(0.0, description="Existing corpus in INR. Must be a plain number. Default is 0.")

    @field_validator("monthly_sip", "existing_corpus", "annual_return_percent", "step_up_percent", mode="before")
    @classmethod
...
class FIRECalculatorInput(BaseModel):
    ...
    monthly_expenses: float = Field(..., description="Current monthly expenses in INR. Must be a plain number, not an expression.")
    ...
    current_corpus: float = Field(..., description="Total current invested corpus in INR. Must be a plain number.")
    monthly_savings: float = Field(..., description="Current monthly savings in INR. Must be a plain pre-computed number, not an expression.")
    inflation_rate: float = Field(6.0, description="Expected annual inflation rate in percent. Default is 6%.")
    pre_retirement_return: float = Field(12.0, description="Expected annual return before retirement in percent. Default 12%.")
    post_retirement_return: float = Field(7.0, description="Expected annual return after retirement in percent. Default 7%.")
    life_expectancy: int = Field(85, description="Life expectancy in years. Default is 85.")
    withdrawal_rate: float = Field(4.0, description="Safe withdrawal rate in percent per year. Default is 4%.")

    @field_validator("monthly_expenses", "current_corpus", "monthly_savings",
                     "inflation_rate", "pre_retirement_return", "post_retirement_return",
                     "withdrawal_rate", mode="before")
    @classmethod
    def coerce_numeric(cls, v: Any) -> float:
        return _safe_eval(v)
