...
def run_terminal_e2e(demo_mode: bool = False) -> None:
    ...
    print("\nRunning FIRE crew...")
    years_to_retire = max(1, profile["retirement_age_target"] - profile["personal"]["age"])
    target_income = profile["expenses"]["monthly_total"]
    inflation = 6.0
    _kickoff_with_retry("FIRE crew", FIRECrew, {
        "profile": profile_str,
    ...
    print("\nRunning Tax crew...")
    deductions = profile["assets"]["ppf"] + profile["assets"]["epf"]
    _kickoff_with_retry("Tax crew", TaxCrew, {
        "profile": profile_str,
        "regime": "new",
    ...
    emergency_fund = profile["risk_profile"]["emergency_fund_months"] * profile["expenses"]["monthly_total"]
    savings_rate = profile["derived"]["savings_rate_percent"]
    _kickoff_with_retry("Health crew", HealthCrew, {
        "profile": profile_str,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run terminal end-to-end flow")
    parser.add_argument("--demo", action="store_true", help="Run in demo mode with pre-filled answers")
    args = parser.parse_args()
    
    run_terminal_e2e(demo_mode=args.demo)
